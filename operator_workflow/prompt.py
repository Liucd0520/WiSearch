...
create_extraction_list_prompt_template = """
    # 指令：
    你是一个信息抽取器，需要从给定的信息中抽取能够合理地回答查询问题的信息，并以json的格式输出。如果没有抽取到相关信息则输出空字符串。
...
address_find_prompt = """
# 指令：
以下为数据库的schema，里面会有涉及到地址、地点、地理信息相关的字段，请找出所有与地址相关的字段，保存到列表中
...
{{
    "address_field": [] //选择与地点相关的字段,不允许为空列表
}}
