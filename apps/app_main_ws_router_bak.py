...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            related_columns = config_dict['related_columns'] 
            distinct_values = config_dict['distinct_values']
            
            abbr_column = config_dict['abbr_column']
            full_abbr_values = config_dict['full_abbr_values'] 
            ...
            if task_type in ['SQL', '内容分类']: 
                # 生成候选sql查询语句,并发生给前端
                obtain_data, sql_result, params = await sql_path(query, use_tables, mysql_db, new_linking_columns, schema, view_values, 
                                                                 sql_gen_chain,  sql_feedback_chain, abbr_column, full_abbr_values,
...
async def load_config(databaseId: int):
    ...
    for selected_table_name in selected_table_names:
        ...
        if len(table_related_columns) != 0:
            related_columns = table_related_columns
            related_use_table = selected_table_name
        
        if len(table_abbr_columns) != 0:
            abbr_columns = table_abbr_columns
            abbr_use_table = selected_table_name
    ...
    logger.info(f'存在缩写的表: {abbr_use_table}')
    logger.info(f'存在关联分析的表: {related_use_table}')
    
    

    # 获取关联字段的值
    distinct_values = get_distinct_values(mysql_db, related_use_table, related_columns)
    # 获取缩写列的所有枚举值
    full_abbr_dict = get_distinct_values(mysql_db, abbr_use_table, abbr_columns)
    if abbr_columns:
        abbr_column = abbr_columns[0]
        full_abbr_values = full_abbr_dict[abbr_columns[0]] # 只允许其中一个列是有值的
    else:
        abbr_column = None 
        full_abbr_values = []
    ...
    config.is_abbr_analysis = 1 if abbr_column else 0
    ...
    config_dict['related_columns'] = related_columns  
    config_dict['distinct_values'] = distinct_values
    config_dict['abbr_column'] = abbr_column 
    config_dict['unstructured_column'] = unstructured_column
    config_dict['full_abbr_values']  = full_abbr_values 
