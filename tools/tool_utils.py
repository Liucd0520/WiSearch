...
def build_schema(mysql_db, mysql_table_name, primary_key_name, columns_map, is_chinese):
    # Create schema
    ...
    for field_name_mysql, raw_field_type in zip(field_list, type_list):
        field_type = raw_field_type.split('(')[0]  # 处理 'varchar(40)' 这种问题
        print(field_name_mysql, field_type)

        field_name = columns_map[field_name_mysql] if not is_chinese else field_name_mysql
        ...
        pr_k = columns_map[primary_key_name] if not is_chinese else primary_key_name
        if field_name == pr_k:
