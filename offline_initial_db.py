...
def init_db(table_names=[], query_list=[], meta_data_chain: Runnable = None, is_meta_data=True, is_schema_linking_gen=True, is_distinct_values_gen=True):

    #step1 生成元数据
    schema_list = []
    if is_meta_data:
        for table_name in table_names:
            schema_with_sample, is_True = meta_data_gen(config.mysql_uri, table_name, meta_data_chain, is_supplement=True)
            schema_gen, _ = split_metadata(schema_with_sample)
