...
def example_preprocess(example_file):
    with open(example_file, 'r', encoding='utf-8') as f_json:
        case_list = json.load(f_json)
    corpus = [each_data['query'] for each_data in case_list]
    embedding_corpus = embedding_bge(corpus)

    return case_list, corpus, embedding_corpus
...
def example_postprocess(case_list, top_indices):
    select_case_list = [corpus[i] for i in top_indices]
    filtered_list = [str(item) for item in case_list if item['query'] in select_case_list]

    return ';'.join(filtered_list), select_case_list
