...
from module.structured_output import create_str_chain
...
def meta_data_gen(mysql_uri: str, table_name: str,  chain: Runnable, is_supplement: bool = True) -> bool:
    ...
    logger.info(f'初始的schema: {schema}')
    logger.info(f"开始利用大模型补全数据")
    new_schema = chain.invoke({'schema': schema, 'enum_values': enum_values, 'samples': samples})       
    logger.info(f"大模型补全数据完成 \n {new_schema}")     
...
if __name__ == '__main__':
    ...
    meta_data_chain = create_str_chain(prompt_meta_data, llm_qwen_14B)
